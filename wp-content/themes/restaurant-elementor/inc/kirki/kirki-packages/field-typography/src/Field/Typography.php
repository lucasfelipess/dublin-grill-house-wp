...
class Typography extends Field {
	...
	public function init( $args = [] ) {
		...
		self::$std_variants = [
			[
				'value' => 'regular',
				'label' => __( 'Regular', 'restaurant-elementor' ),
			],
			[
				'value' => 'italic',
				'label' => __( 'Italic', 'restaurant-elementor' ),
			],
			[
				'value' => '700',
				'label' => __( '700', 'restaurant-elementor' ),
			],
			[
				'value' => '700italic',
				'label' => __( '700 Italic', 'restaurant-elementor' ),
			],
		];
		...
		self::$complete_variants = [
			...
			[
				'value' => '100',
				'label' => __( '100', 'restaurant-elementor' ),
			],
			[
				'value' => '100italic',
				'label' => __( '100 Italic', 'restaurant-elementor' ),
			],
			[
				'value' => '200',
				'label' => __( '200', 'restaurant-elementor' ),
			],
			[
				'value' => '200italic',
				'label' => __( '200 Italic', 'restaurant-elementor' ),
			],
			[
				'value' => '300',
				'label' => __( '300', 'restaurant-elementor' ),
			],
			[
				'value' => '300italic',
				'label' => __( '300 Italic', 'restaurant-elementor' ),
			],
			[
				'value' => '500',
				'label' => __( '500', 'restaurant-elementor' ),
			],
			[
				'value' => '500italic',
				'label' => __( '500 Italic', 'restaurant-elementor' ),
			],
			[
				'value' => '600',
				'label' => __( '600', 'restaurant-elementor' ),
			],
			[
				'value' => '600italic',
				'label' => __( '600 Italic', 'restaurant-elementor' ),
			],
			[
				'value' => '700',
				'label' => __( '700', 'restaurant-elementor' ),
			...
			[
				'value' => '800',
				'label' => __( '800', 'restaurant-elementor' ),
			],
			[
				'value' => '800italic',
				'label' => __( '800 Italic', 'restaurant-elementor' ),
			],
			[
				'value' => '900',
				'label' => __( '900', 'restaurant-elementor' ),
			],
			[
				'value' => '900italic',
				'label' => __( '900 Italic', 'restaurant-elementor' ),
			],
		];
	...
	private function add_sub_fields( $args ) {
		...
		if ( isset( $args['default']['font-family'] ) ) {
			...
			new \Kirki\Field\ReactSelect(
				wp_parse_args(
					[
						'label'       => esc_html__( 'Font Family', 'restaurant-elementor' ),
						'description' => '',
			...
			new \Kirki\Field\ReactSelect(
				wp_parse_args(
					[
						'label'       => esc_html__( 'Font Variant', 'restaurant-elementor' ),
						'description' => '',
		...
		if ( $font_size_field_specified || $color_field_specified ) {
			$group = [
				'font-size' => [
					'type'         => 'dimension',
					'label'        => esc_html__( 'Font Size', 'restaurant-elementor' ),
					'is_specified' => $font_size_field_specified,
				],
				'color'     => [
					'type'         => 'react-colorful',
					'label'        => esc_html__( 'Font Color', 'restaurant-elementor' ),
		...
		if ( $text_align_field_specified || $text_transform_field_specified ) {
			$group = [
				'text-align'     => [
					'type'         => 'react-select',
					'label'        => esc_html__( 'Text Align', 'restaurant-elementor' ),
					'is_specified' => $text_align_field_specified,
					'choices'      => [
						'initial' => esc_html__( 'Initial', 'restaurant-elementor' ),
						'left'    => esc_html__( 'Left', 'restaurant-elementor' ),
						'center'  => esc_html__( 'Center', 'restaurant-elementor' ),
						'right'   => esc_html__( 'Right', 'restaurant-elementor' ),
						'justify' => esc_html__( 'Justify', 'restaurant-elementor' ),
					],
				],
				'text-transform' => [
					'type'         => 'react-select',
					'label'        => esc_html__( 'Text Transform', 'restaurant-elementor' ),
					'is_specified' => $text_transform_field_specified,
					'choices'      => [
						'none'       => esc_html__( 'None', 'restaurant-elementor' ),
						'capitalize' => esc_html__( 'Capitalize', 'restaurant-elementor' ),
						'uppercase'  => esc_html__( 'Uppercase', 'restaurant-elementor' ),
						'lowercase'  => esc_html__( 'Lowercase', 'restaurant-elementor' ),
					],
				],
			];
		...
		if ( $text_decoration_field_specified ) {
			$group = [
				'text-decoration' => [
					'type'         => 'react-select',
					'label'        => esc_html__( 'Text Decoration', 'restaurant-elementor' ),
					'is_specified' => $text_decoration_field_specified,
					'choices'      => [
						'none'         => esc_html__( 'None', 'restaurant-elementor' ),
						'underline'    => esc_html__( 'Underline', 'restaurant-elementor' ),
						'line-through' => esc_html__( 'Line Through', 'restaurant-elementor' ),
						'overline'     => esc_html__( 'Overline', 'restaurant-elementor' ),
						'solid'        => esc_html__( 'Solid', 'restaurant-elementor' ),
						'wavy'         => esc_html__( 'Wavy', 'restaurant-elementor' ),
					],
				],
			];
		...
		if ( $line_height_field_specified || $letter_spacing_field_specified ) {
			$group = [
				'line-height'    => [
					'type'         => 'dimension',
					'label'        => esc_html__( 'Line Height', 'restaurant-elementor' ),
					'is_specified' => $line_height_field_specified,
				],
				'letter-spacing' => [
					'type'         => 'dimension',
					'label'        => esc_html__( 'Letter Spacing', 'restaurant-elementor' ),
		...
		if ( $margin_top_field_specified || $margin_bottom_field_specified ) {
			$group = [
				'margin-top'    => [
					'type'         => 'dimension',
					'label'        => esc_html__( 'Margin Top', 'restaurant-elementor' ),
					'is_specified' => $margin_top_field_specified,
				],
				'margin-bottom' => [
					'type'         => 'dimension',
					'label'        => esc_html__( 'Margin Bottom', 'restaurant-elementor' ),
	...
	private function get_font_family_choices() {
		...
		$choices['default'] = [
			esc_html__( 'Defaults', 'restaurant-elementor' ),
			[
				'' => esc_html__( 'Default', 'restaurant-elementor' ),
			],
		];
		...
		$choices['standard'] = [
			esc_html__( 'Standard Fonts', 'restaurant-elementor' ),
			$std_fonts,
		];

		$choices['google'] = [
			esc_html__( 'Google Fonts', 'restaurant-elementor' ),

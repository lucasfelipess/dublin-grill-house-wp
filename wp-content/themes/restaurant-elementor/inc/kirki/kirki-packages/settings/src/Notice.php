...
class Notice {
	...
	public function dismiss_discount_notice() {
		...
		if ( ! wp_verify_nonce( $nonce, 'Kirki_Dismiss_Discount_Notice' ) ) {
			wp_send_json_error( __( 'Invalid nonce', 'restaurant-elementor' ) );
		}

		if ( ! current_user_can( 'manage_options' ) ) {
			wp_send_json_error( __( "You don't have capability to run this action", 'restaurant-elementor' ) );
		}
		...
		wp_send_json_success( __( 'Discount notice has been dismissed', 'restaurant-elementor' ) );
	}

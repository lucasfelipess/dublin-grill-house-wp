...
class Repeater extends Base {
	...
	public function __construct( $manager, $id, $args = [] ) {
		...
		$this->row_label = [
			'type'  => 'text',
			'value' => esc_attr__( 'row', 'restaurant-elementor' ),
			'field' => false,
		];
		...
		if ( empty( $this->button_label ) ) {
			...
			$this->button_label = sprintf( esc_html__( 'Add new %s', 'restaurant-elementor' ), $this->row_label['value'] );
		}
					...
					case 'dropdown-pages':
						...
						$dropdown = wp_dropdown_pages(
							[
								'name'              => '',
								'echo'              => 0,
								'show_option_none'  => esc_html__( 'Select a Page', 'restaurant-elementor' ),
								'option_none_value' => '0',
								'selected'          => '',
							]
						);
	...
	protected function render_content() {
		...
		<?php if ( isset( $this->choices['limit'] ) ) : ?>
			...
			<p class="limit"><?php printf( esc_html__( 'Limit: %s rows', 'restaurant-elementor' ), esc_html( $this->choices['limit'] ) ); ?></p>
		<?php endif; ?>
		...
		<script type="text/html" class="customize-control-repeater-content">
			...
			<li class="repeater-row minimized" data-row="{{{ index }}}">
				...
				<div class="repeater-row-content">
					<# _.each( data, function( field, i ) { #>
						...
						<div class="repeater-field repeater-field-{{{ field.type }}} repeater-field-{{ field.id }}">
							...
							<# } else if ( field.type === 'image' || field.type === 'cropped_image' ) { #>
								...
								<figure class="kirki-image-attachment" data-placeholder="<?php esc_attr_e( 'No Image Selected', 'restaurant-elementor' ); ?>" >
									<# if ( field.default ) { #>
										<# var defaultImageURL = ( field.default.url ) ? field.default.url : field.default; #>
										<img src="{{{ defaultImageURL }}}">
									<# } else { #>
										<?php esc_html_e( 'No Image Selected', 'restaurant-elementor' ); ?>
									<# } #>
								</figure>

								<div class="actions">
									<button type="button" class="button remove-button<# if ( ! field.default ) { #> hidden<# } #>"><?php esc_html_e( 'Remove', 'restaurant-elementor' ); ?></button>
									<button type="button" class="button upload-button" data-label=" <?php esc_attr_e( 'Add Image', 'restaurant-elementor' ); ?>" data-alt-label="<?php echo esc_attr_e( 'Change Image', 'restaurant-elementor' ); ?>" >
										<# if ( field.default ) { #>
											<?php esc_html_e( 'Change Image', 'restaurant-elementor' ); ?>
										<# } else { #>
											<?php esc_html_e( 'Add Image', 'restaurant-elementor' ); ?>
										<# } #>
									</button>
									<# if ( field.default.id ) { #>
							...
							<# } else if ( field.type === 'upload' ) { #>
								...
								<figure class="kirki-file-attachment" data-placeholder="<?php esc_attr_e( 'No File Selected', 'restaurant-elementor' ); ?>" >
									<# if ( field.default ) { #>
									...
									<# } else { #>
										<?php esc_html_e( 'No File Selected', 'restaurant-elementor' ); ?>
									<# } #>
								</figure>

								<div class="actions">
									<button type="button" class="button remove-button<# if ( ! field.default ) { #> hidden<# } #>"><?php esc_html_e( 'Remove', 'restaurant-elementor' ); ?></button>
									<button type="button" class="button upload-button" data-label="<?php esc_attr_e( 'Add File', 'restaurant-elementor' ); ?>" data-alt-label="<?php esc_attr_e( 'Change File', 'restaurant-elementor' ); ?>">
										<# if ( field.default ) { #>
											<?php esc_html_e( 'Change File', 'restaurant-elementor' ); ?>
										<# } else { #>
											<?php esc_html_e( 'Add File', 'restaurant-elementor' ); ?>
										<# } #>
									</button>
									<# if ( field.default.id ) { #>
										<input type="hidden" class="hidden-field" value="{{{ field.default.id }}}" data-field="{{{ field.id }}}" >
					...
					<# }); #>
					<button type="button" class="button-link repeater-row-remove"><?php esc_html_e( 'Remove', 'restaurant-elementor' ); ?></button>
				</div>
			</li>
		</script>

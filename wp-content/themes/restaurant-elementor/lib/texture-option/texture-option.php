...
class restaurant_elementor_theme_option{
...
function menu_tab() {
    $menu_title = esc_html__('Restaurant Elementor Options', 'restaurant-elementor');
    add_theme_page( esc_html__( 'Restaurant Elementor', 'restaurant-elementor' ), $menu_title, 'edit_theme_options', 'texture_started',array($this,'tab_page'));
...
wp_enqueue_style( 'restaurant-elementor-started-css', get_template_directory_uri() . '/lib/texture-option/assets/css/started.css' );
wp_enqueue_script( 'restaurant-elementor-admin-load', get_template_directory_uri() . '/lib/texture-option/assets/js/texture-options.js', array('jquery', 'updates' ), true );
...
$data = apply_filters(
    ...
    wp_localize_script( 'restaurant-elementor-admin-load', 'THAdmin', $data); 


} else {

    wp_enqueue_style( 'restaurant-elementor-notify-css', get_template_directory_uri() . '/lib/restaurant-elementor-option/assets/css/notify.css' );  
...
function tab_constant(){
    ...
    $tab_array = array();
    $tab_array['header'] = array('theme_brand' => __('wptexture','restaurant-elementor'),
    'theme_brand_url' => esc_url($themedata->get( 'AuthorURI' )),
    'welcome'=>sprintf(esc_html__('Welcome To %1s Theme', 'restaurant-elementor'), esc_html($themedata->get( 'Name' ),'restaurant-elementor'), $themedata->get( 'Version' ) ),
    'welcome_desc' => esc_html($themedata->get( 'Name' ).' is an eCommerce WordPress theme specially made for shopping websites.', 'restaurant-elementor' ),
...
if(empty($pages)){
      $post_id = wp_insert_post(array (
       'post_type' => 'page',
       'post_title' => __('Home Page','restaurant-elementor'),
       'post_content' => '',
       'post_status' => 'publish',
     ...
     /*
      ...
      if ( ! current_user_can( 'install_plugins' ) || ! isset( $_POST['init'] ) || ! $_POST['init'] ) {
        wp_send_json_error(
          array(
            'success' => false,
            'message' => __( 'No plugin specified', 'restaurant-elementor' ),
          )
        );
      }
      ...
      wp_send_json_success(
        array(
          'success' => true,
          'message' => __( 'Plugin Successfully Activated', 'restaurant-elementor' ),
        )
      );
...
function plugin_install_button($plugin){
            ...
            <h4>'.esc_html( $plugin['plugin_name'] ). ' </h4><a class="plugin-detail thickbox open-plugin-details-modal" href="'.esc_url( $plugin['detail_link'] ).'">'.esc_html__( 'Details & Version', 'restaurant-elementor' ).'</a>
            </div>';
        ...
        foreach($recommend_plugins[0] as $slug=>$plugin){
            ...
            $button_class = 'install-now button '.$slug;
             ...
             if ( is_plugin_active( $plugin_init ) ) {
                   ...
                   $button_txt = esc_html__( 'Activated', 'restaurant-elementor' );
                   $detail_link = $install_url = '';
                }

            if ( ! is_plugin_active( $plugin_init ) ){
                    $button_txt = esc_html__( 'Install Now', 'restaurant-elementor' );
                    if ( ! $status ) {
                        ...
                        $button_txt = esc_html__( 'Activate Now', 'restaurant-elementor' );
                    }
                }
...
$boj = new restaurant_elementor_theme_option(); ?>